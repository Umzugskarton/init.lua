        ...
        config = function()
            require("mason-nvim-dap").setup({
                ...
                handlers = {
                    ...
                    -- We manually handled "pwa-node" above, so we don't need a specific handler here
                    -- unless you want to override the default "js-debug-adapter" setup.
                    delve = function(config)
                        table.insert(config.configurations, 1, {
                            args = function() return vim.split(vim.fn.input("args> "), " ") end,
                            type = "delve", name = "file", request = "launch", program = "${file}", outputMode = "remote",
                        })
                        require("mason-nvim-dap").default_setup(config)
                    end,
                },
